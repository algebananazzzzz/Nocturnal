...
const ProjectPage = () => {
  ...
  const getComponent = (type, value) => {
    if (type === "svgText") {
      return <Text svgFill={value.svg.fill} contrastLevel={value.svg.contrastLevel} svgSizeSm={value.svg.sizeSm} svgSizeMd={value.svg.sizeMd} svgViewBox={value.svg.viewbox} text={value.text} link={value.link}>
        {parse(value.svg.path)}
      </Text>
    } else if (type === "text") {
      return <Text classes="mt-5" text={value}>
      </Text>
    } else if (type === "svgListed") {
    ...
    <HomeLayout>
      <body class="min-h-screen dark:bg-slate-900">
        ...
        <div class="max-w-7xl px-4 sm:px-6 lg:px-8 mx-auto">
          ...
          <div class="mt-10 lg:mt-0 grid md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {PageContent.projects.map((value) => {
              const [[name, content]] = Object.entries(value);

              return <div class="group flex flex-col bg-white hover:bg-gray-200 hover:border-gray-300 border dark:border-gray-700 dark:hover:bg-gray-600 dark:hover:border-gray-500 transition-all duration-250  shadow-sm rounded-md px-4 md:px-5 dark:bg-gray-800 dark:shadow-slate-700/[.7]">
                <div className="child block group-hover:hidden py-4 md:py-5">
                  <p class="text-sm font-medium text-fuchsia-500 dark:text-fuchsia-500">
                    {content.date}
                  </p>
                  ...
                  <div className="mt-2 flex items-center gap-x-5 p-2">
                    {content.svgIcons.map((value) => {
                      return getComponent("svgListed", value)
                    })}
                  </div>
                </div>
                <div className="child hidden group-hover:block py-3 md:py-4">
                  <p class="text-gray-800 dark:text-gray-300 text-justify">
                    {content.additional_description}
                  </p>
                  <div className="mt-2 flex gap-x-2">
                    {content.actions.map((value) => {
                      return getComponent("action", value)
                    })}
                  </div>
                </div>
              </div>
            })}
            <div class="group flex flex-col bg-white hover:bg-gray-200 hover:border-gray-300 border dark:border-gray-700 dark:hover:bg-gray-600 dark:hover:border-gray-500 transition-all duration-250  shadow-sm rounded-md px-4 md:px-5 dark:bg-gray-800 dark:shadow-slate-700/[.7]">
              <div className="child block group-hover:hidden py-4 md:py-5">
                <p class="text-sm font-medium uppercase text-fuchsia-500 dark:text-fuchsia-500">
                  2020 - 2022
                </p>
                <Text classes="mt-1 font-bold" sizeSm="md" sizeMd="xl" contrastLevel={1} text="Others"></Text>
                <Text classes="mt-2" sizeMd="base" text="Miscellaneous websites and scripts created to address specific user stories, in my NS and JC life"></Text>

              </div>
              <div className="child hidden group-hover:block py-3 md:py-4">
                <p class="text-gray-800 dark:text-gray-300 text-justify">
                  <Action link="https://github.com/algebananazzzzz/algebananazzzzz.github.io/blob/main/cdoViper/EqualiDuty/README.md" text="EqualityDuty"></Action>
                  &nbsp;to automate distributing and resolving schedule clashes in duty planning
                </p>
                <p class="text-gray-800 dark:text-gray-300 text-justify">
                  <Action link="https://github.com/algebananazzzzz/wordcount_4_PW" text="WordCounter"></Action>
                  &nbsp;to count words in H2 PW
                </p>
                <p class="mt-2 text-gray-800 dark:text-gray-300 text-justify">
                  <Action link="https://algebananazzzzz.github.io/organic-chemistry-mindmap.html" text="Mindmap" color="purple"></Action>
                  &nbsp;to efficiently convey knowledge about H2 Organic Chemistry to my peers
                </p>
                {/* <div className="mt-2 flex gap-x-2">
                  <Link to="/motivations">
                    <span class="inline-flex items-center py-1.5 px-3 rounded-sm text-sm font-medium bg-indigo-500 hover:bg-indigo-700 dark:bg-indigo-600 dark:hover:bg-indigo-800 text-white">View More</span>
                  </Link>
                </div> */}
              </div>
            </div>
          </div>
          <Text classes="mt-5" text=" Upcoming...">
          </Text>
          <div class="mt-5 lg:mt-7 grid sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {PageContent.upcoming_projects.map((value) => {
              const [[name, content]] = Object.entries(value);
              return <div class="flex flex-col bg-white border shadow-sm rounded-md p-4 md:p-5 dark:bg-gray-800 dark:border-gray-700 dark:shadow-slate-700/[.7]">
                <p class="text-sm font-medium uppercase text-fuchsia-500 dark:text-fuchsia-500">
                  {content.date}
                </p>
                <Text classes="mt-1 font-bold" sizeSm="md" sizeMd="xl" contrastLevel={1} text={name}></Text>
